"""
これは記憶辞書の実装クラスです。
This is a implementation class of "Memory Dictionary".
"""
...
class MemoryDictionary(nn.Module):
    ...
    """
    def __init__(self,num_memory:int, num_dims:int,device:torch.device="cpu",dtype:torch.dtype=torch.float, 
        ...
        weight1 = torch.randn((num_memory,num_dims),device=device,dtype=dtype)
    ...
    def add_memories(self, num:int) -> None:
        ...
        newT = new.T
        new_ids = torch.arange(self.num_memory,self.num_memory+num,dtype=torch.int64)
        self.num_memory += num
    ...
    def get_memory_vector(self, indices:torch.Tensor, requires_grad:bool = False) -> torch.Tensor:
    ...
    @torch.no_grad()
    def calculate_lr(self, v:torch.Tensor, g:torch.Tensor, U:torch.Tensor, H:torch.Tensor, tgt_idx:int) -> float:
        """
    ...
    def binary_search_lr(
        self, min_lr:float, max_lr:float, 
        A_p:torch.Tensor, B_p:torch.Tensor, C_p:torch.Tensor, num_steps:int) -> float:
        """
    ...
    def __getitem__(self, indices:Union[int,torch.Tensor]) -> Union[int, torch.Tensor]:
        """
